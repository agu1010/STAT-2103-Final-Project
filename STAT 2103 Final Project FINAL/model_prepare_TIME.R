setwd("~/Downloads/STAT 2103 Final Project")
rm(list = ls())
library(data.table)
set.seed(2)
...
# Load data
polls_100days <- preprocess_polls_f( data.table(read.delim("../Documents/STAT 2103 Final Project Desktop/polls_auxiliary_dataset.tsv") ) )
...
polls_100days$poll_srs_result <- sim_srs_result_f(polls_100days$v_r, polls_100days$n_i)

# Write data for each election type into a separate RData file
for (elec in c('Senatorial','Gubernatorial','Presidential')) {
  election_polls <- polls_100days[election_type == elec]
  
  # Generate corresponding vector of r[i]'s for input to Stan model
  ...
  file_name <- sprintf("../Documents/STAT 2103 Final Project Desktop/stan_data_time_%s.RData", tolower(elec))
